...
void InkScreenBWR::begin(const char cs_W21, const char cs_GT30, const char dc)
{
	SPI.begin();
...
void InkScreenBWR::spiDelay(unsigned char xsp){
	unsigned char i;
	while(xsp--)
	{
...
void InkScreenBWR::spiRead(uint32_t addr, uint8_t len, char *ch)
{
	DFR_GT30_CS_0;
	DFR_W21_CS_1;
	SPI.transfer(0x0b);
...
void InkScreenBWR::spiWriteByte(unsigned char value){
	
	SPI.transfer(value);
}

void InkScreenBWR::wirteCmd(unsigned char command){
	spiDelay(1);
	DFR_W21_CS_0;
	DFR_GT30_CS_1;
	DFR_W21_DC_0;
...
void InkScreenBWR::wirteData(unsigned char data){
	spiDelay(1);
	DFR_W21_CS_0;
	DFR_GT30_CS_1;
	DFR_W21_DC_1;
...
void InkScreenBWR::setWindow(uint16_t x, uint16_t y){
...
void InkScreenBWR::powerOn(void)
{
...
void InkScreenBWR::powerOff(void)
{
	wirteCmd(0x12);              
...
void InkScreenBWR::picDisplay(const unsigned char *pic_bw, const unsigned char *pic_red)
{
...
InkScreen_Error InkScreenBWR::clear(uint8_t color)
{
	uint8_t bw,red;
	if(color == WHITE)     {bw = 0x00; red = 0x00;}
	else if(color == RED)  {bw = 0x00; red = 0xff;}
	else if(color == BLACK){bw = 0xff; red = 0x00;}
	else{
...
InkScreen_Error InkScreenBWR::drawPoint(const unsigned char x, const unsigned char y, const unsigned char color)
{
	uint8_t sx,sy;
	uint16_t sby;
	if(x>211 || y>103){
	...
	if(color == RED){//Red dot
		...
		return true;
	}else if(color == LUCENCY) return true;
	else return false;
}

InkScreen_Error InkScreenBWR::drawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t color)
{
...
InkScreen_Error InkScreenBWR::drawRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t color)
{
	InkScreen_Error Status = true;
...
InkScreen_Error InkScreenBWR::drawCirclePoint( uint16_t xc, uint16_t yc, uint16_t x, uint16_t y, uint8_t color)
{
	InkScreen_Error Status = true;
	uint8_t Ix=1,Iy=1;
	for(uint8_t i=0; i<8; i++){
		if(i<4) {Status = drawPoint(xc + x*Ix, yc + y*Iy, color); if(!Status) return Status;}
		else {Status = drawPoint(xc + y*Ix, yc + x*Iy, color); if(!Status) return Status;}
		Ix *= -1;
		if((i+1)%2==0){
			Iy *= -1;
		}	
...
InkScreen_Error InkScreenBWR::drawCircle(uint16_t xc, uint16_t yc, uint16_t r, uint16_t fill, uint8_t color)
{
	InkScreen_Error Status = true;
	int x=0, y=r, yi,d;
	d = 3-2*r;
...
InkScreen_Error InkScreenBWR::unicodeToGB2312(char *unicode, char *GB2312) 
{
	uint32_t addr;
	uint8_t result=0;
	uint32_t h;
...
uint32_t InkScreenBWR::GB2312_addr(char *ch, uint8_t type)
{
	uint32_t temp = 0;
	if(ch[0]<0x80){
		if( ch[0] >= ' ' )                  {temp = ch[0] - ' ';}
...
void InkScreenBWR::disStr(uint8_t x, uint8_t y, char *ch, uint8_t color) 
{
	char unicode_hz[2] = {0};
	char gb2312_hz [2] = {0};
	char temp_char[2] = {0};
	...
	if(character_type == CHARACTER_TYPE_UTF8) {
		while(*ch) {
...
InkScreen_Error InkScreenBWR::showStr(uint8_t x, uint8_t y, uint8_t size, char *ch, uint8_t color)
{
	uint8_t sx=x,font;
